...
关键参数赋值
"""
epsilon_start, epsilon_end, epsilon_decay = 0.7, 0.1, 6000000
gamma = 0.99
...
def train(env):
    ...
    global_step = 0
    writer = SummaryWriter(log_dir='runs/dqn4')
    ...
    # 创建实例
    optimizer = optim.Adam(qnet.parameters(), lr=0.9e-4)
    buffer = ReplayBuffer()
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        if (episode + 1) % 500 == 0:
            torch.save(qnet.state_dict(), f'runs/dqn4_ckpt_{episode+1}.pth')
